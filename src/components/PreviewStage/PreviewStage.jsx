...
export default function PreviewStage({ scene, replay }) {
  ...
  return <section className={styles.card}>
    ...
    <div data-animation-preview ref={viewport} className={styles.viewport} tabIndex={scroll ? 0 : undefined} role="region" aria-label="Animation preview">
      <style>{scene.map(element => generateCss({ ...element.config, name: `preview_${element.id}` }, '[data-animation-preview] ')).join('\n')}</style>
