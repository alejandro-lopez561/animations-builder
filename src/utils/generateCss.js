...
import { specialCss, contentCss } from './specialCss.js'
...
export function generateCss(config, scope = '', appearance) {
  ...
  if (motions[config.motion]?.elementType) return specialCss(config, scope, layout, appearance)
  return `${contentCss(config, appearance, base)}\n${base} {
  ${layout}
...
` : ''}${active} {
  animation: ${name} ${timing(config, appearance).cycle}ms ${config.easing} ${config.delay || 0}ms ${config.iterations || 1} ${config.direction || 'normal'} both;
}
